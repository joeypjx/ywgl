...
bool DatabaseManager::initializeNodeTables() {
    ...
    try {
        ...
        // 创建索引以提高查询性能
        db_->exec("CREATE INDEX IF NOT EXISTS idx_node_cpu_metrics_hostIp ON node_cpu_metrics(hostIp)");
        db_->exec("CREATE INDEX IF NOT EXISTS idx_node_cpu_metrics_timestamp ON node_cpu_metrics(timestamp)");
        db_->exec("CREATE INDEX IF NOT EXISTS idx_node_memory_metrics_hostIp ON node_memory_metrics(hostIp)");
        db_->exec("CREATE INDEX IF NOT EXISTS idx_node_memory_metrics_timestamp ON node_memory_metrics(timestamp)");
        db_->exec("CREATE INDEX IF NOT EXISTS idx_node_disk_metrics_hostIp ON node_disk_metrics(hostIp)");
        db_->exec("CREATE INDEX IF NOT EXISTS idx_node_disk_metrics_timestamp ON node_disk_metrics(timestamp)");
        db_->exec("CREATE INDEX IF NOT EXISTS idx_node_network_metrics_hostIp ON node_network_metrics(hostIp)");
        db_->exec("CREATE INDEX IF NOT EXISTS idx_node_network_metrics_timestamp ON node_network_metrics(timestamp)");
        db_->exec("CREATE INDEX IF NOT EXISTS idx_node_docker_metrics_hostIp ON node_docker_metrics(hostIp)");
        db_->exec("CREATE INDEX IF NOT EXISTS idx_node_docker_metrics_timestamp ON node_docker_metrics(timestamp)");
        db_->exec("CREATE INDEX IF NOT EXISTS idx_node_gpu_metrics_hostIp ON node_gpu_metrics(hostIp)");
...
bool DatabaseManager::updateNodeStatusOnly(const std::string& hostIp, const std::string& new_status) {
    if (!db_) {
    ...
    try {
        ...
        std::cerr << "Error setting node status for host " << hostIp << ": " << e.what() << std::endl;
        return false;
    }
}
...
void DatabaseManager::nodeStatusMonitorLoop() {
    ...
    while (node_status_monitor_running_.load()) {
        try {
            ...
            SQLite::Statement query(*db_, "SELECT hostIp, updated_at, status FROM node");
            ...
            while (query.executeStep()) {
                ...
                if (current_status != "empty" && current_status != "offline") {
                    if ((now_epoch - last_updated_at) > 5) { 
                        std::cout << "Node with hostIp " << hostIp 
                                  << " is inactive. Setting status to offline via DatabaseManager." << std::endl;
                        this->updateNodeStatusOnly(hostIp, "offline"); 
                    }
                }
            }
...
// 保存slot CPU指标
bool DatabaseManager::saveNodeCpuMetrics(const std::string& hostIp,
    ...
    try {
        ...
        SQLite::Statement insert(*db_,
            "INSERT INTO node_cpu_metrics (hostIp, timestamp, usage_percent, load_avg_1m, load_avg_5m, load_avg_15m, core_count) "
            "VALUES (?, ?, ?, ?, ?, ?, ?)");
...
// 保存slot内存指标
bool DatabaseManager::saveNodeMemoryMetrics(const std::string& hostIp,
    ...
    try {
        ...
        // 插入内存指标
        SQLite::Statement insert(*db_,
            "INSERT INTO node_memory_metrics (hostIp, timestamp, total, used, free, usage_percent) "
            "VALUES (?, ?, ?, ?, ?, ?)");
        insert.bind(1, hostIp);
        insert.bind(2, static_cast<int64_t>(timestamp));
...
// 保存slot磁盘指标
bool DatabaseManager::saveNodeDiskMetrics(const std::string& hostIp,
    ...
    try {
        ...
        SQLite::Statement insert_metrics(*db_,
            "INSERT INTO node_disk_metrics (hostIp, timestamp, disk_count) VALUES (?, ?, ?)");
...
// 保存slot网络指标
bool DatabaseManager::saveNodeNetworkMetrics(const std::string& hostIp,
    ...
    try {
        ...
        SQLite::Statement insert_metrics(*db_,
            "INSERT INTO node_network_metrics (hostIp, timestamp, network_count) VALUES (?, ?, ?)");
        insert_metrics.bind(1, hostIp);
        insert_metrics.bind(2, static_cast<int64_t>(timestamp));
...
// 保存slot GPU指标
bool DatabaseManager::saveNodeGpuMetrics(const std::string& hostIp,
    ...
    try {
        ...
        SQLite::Statement insert_metrics(*db_,
            "INSERT INTO node_gpu_metrics (hostIp, timestamp, gpu_count) VALUES (?, ?, ?)");
        insert_metrics.bind(1, hostIp);
        insert_metrics.bind(2, static_cast<int64_t>(timestamp));
...
// 保存slot Docker指标
bool DatabaseManager::saveNodeDockerMetrics(const std::string& hostIp,
    ...
    try {
        ...
        // 插入Docker指标
        SQLite::Statement insert(*db_,
            "INSERT INTO node_docker_metrics (hostIp, timestamp, container_count, running_count, paused_count, stopped_count) "
            "VALUES (?, ?, ?, ?, ?, ?)");
        insert.bind(1, hostIp);
        insert.bind(2, static_cast<int64_t>(timestamp));
...
// 获取slot CPU指标
nlohmann::json DatabaseManager::getNodeCpuMetrics(const std::string& hostIp, int limit) {
    try {
        nlohmann::json result = nlohmann::json::array();
        ...
        SQLite::Statement query(*db_,
            "SELECT timestamp, usage_percent, load_avg_1m, load_avg_5m, load_avg_15m, core_count "
            "FROM node_cpu_metrics WHERE hostIp = ? ORDER BY timestamp DESC LIMIT ?");
        query.bind(1, hostIp);
        query.bind(2, limit);
...
// 获取slot内存指标
nlohmann::json DatabaseManager::getNodeMemoryMetrics(const std::string& hostIp, int limit) {
    try {
        nlohmann::json result = nlohmann::json::array();
        ...
        SQLite::Statement query(*db_,
            "SELECT timestamp, total, used, free, usage_percent "
            "FROM node_memory_metrics WHERE hostIp = ? ORDER BY timestamp DESC LIMIT ?");
        query.bind(1, hostIp);
        query.bind(2, limit);
...
// 获取slot磁盘指标
nlohmann::json DatabaseManager::getNodeDiskMetrics(const std::string& hostIp, int limit) {
    try {
        nlohmann::json result = nlohmann::json::array();

        // 查询slot_disk_metrics
        SQLite::Statement query(*db_,
            "SELECT id, timestamp, disk_count FROM node_disk_metrics WHERE hostIp = ? ORDER BY timestamp DESC LIMIT ?");
        query.bind(1, hostIp);
        query.bind(2, limit);
...
// 获取slot网络指标
nlohmann::json DatabaseManager::getNodeNetworkMetrics(const std::string& hostIp, int limit) {
    try {
        nlohmann::json result = nlohmann::json::array();

        // 查询slot_network_metrics
        SQLite::Statement query(*db_,
            "SELECT id, timestamp, network_count FROM node_network_metrics WHERE hostIp = ? ORDER BY timestamp DESC LIMIT ?");
        query.bind(1, hostIp);
        query.bind(2, limit);
...
// 获取slot GPU指标
nlohmann::json DatabaseManager::getNodeGpuMetrics(const std::string& hostIp, int limit) {
    try {
        nlohmann::json result = nlohmann::json::array();

        // 查询slot_gpu_metrics
        SQLite::Statement query(*db_,
            "SELECT id, timestamp, gpu_count FROM node_gpu_metrics WHERE hostIp = ? ORDER BY timestamp DESC LIMIT ?");
        query.bind(1, hostIp);
        query.bind(2, limit);
...
// 获取slot Docker指标
nlohmann::json DatabaseManager::getNodeDockerMetrics(const std::string& hostIp, int limit) {
    try {
        nlohmann::json result = nlohmann::json::array();
        ...
        SQLite::Statement query(*db_,
            "SELECT id, timestamp, container_count, running_count, paused_count, stopped_count "
            "FROM node_docker_metrics WHERE hostIp = ? ORDER BY timestamp DESC LIMIT ?");
        query.bind(1, hostIp);
        query.bind(2, limit);
...
bool DatabaseManager::saveNodeResourceUsage(const nlohmann::json& resource_usage) {
    ...
    if (resource.contains("cpu")) {
        saveNodeCpuMetrics(hostIp, timestamp, resource["cpu"]);
    }
    if (resource.contains("memory")) {
        saveNodeMemoryMetrics(hostIp, timestamp, resource["memory"]);
    }
    if (resource.contains("disk")) {
        saveNodeDiskMetrics(hostIp, timestamp, resource["disk"]);
    }
    if (resource.contains("network")) {
        saveNodeNetworkMetrics(hostIp, timestamp, resource["network"]);
    }
    if (resource.contains("docker")) {
        saveNodeDockerMetrics(hostIp, timestamp, resource["docker"]);
    }
    return true;
}